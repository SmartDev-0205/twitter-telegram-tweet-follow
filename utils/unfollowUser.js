...
async function unfollowUser(userId, cookiesFilePath) {

  try {
    const cookies = require(cookiesFilePath);
    const result = await fetch("https://twitter.com/i/api/1.1/friendships/destroy.json", {
      "headers": {
    ...
    if (result.status !== 200) {
      console.log('Error unfollowing user. Status:', result.status);
      return false;
    }
